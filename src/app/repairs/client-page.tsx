...
// app/repairs/client-page.tsx

import React, { useState, useEffect, useMemo } from 'react';
import { RepairCase } from '@/libs/microcms'; // 型定義をインポート
    ...
    <a href={`/repairs/${repairCase.slug}`} className="block">
      <div className="relative">
        <img 
          ...
          onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/400x250/E0E0E0/333333?text=No+Image'; }}
        />
        {repairCase.category && (
          <div className="absolute top-2 left-2 bg-blue-600 text-white text-xs font-bold px-2 py-1 rounded">{repairCase.category.name}</div>
        )}
      </div>
      <div className="p-4">
        <h3 className="font-bold text-lg text-gray-800 group-hover:text-blue-600 transition-colors">{repairCase.title}</h3>
        <div className="mt-3 flex flex-wrap gap-2">
          {repairCase.tags?.map(tag => (
            <span key={tag.id} className="text-xs bg-gray-200 text-gray-700 px-2 py-1 rounded-full">{tag.name}</span>
          ))}
        </div>
      </div>
    </a>
...
// これは「クライアントコンポーネント」です。
// ユーザーのブラウザ上で動作し、絞り込みなどの操作を担当します。
...
  const [filteredExamples, setFilteredExamples] = useState<RepairCase[]>(initialRepairs);
  ...
  // microCMSから取得したデータに含まれるカテゴリとタグのリストを生成
  const allCategories = useMemo(() => Array.from(new Set(initialRepairs.map(ex => ex.category?.name).filter(Boolean))), [initialRepairs]);
  const allSymptoms = useMemo(() => Array.from(new Set(initialRepairs.flatMap(ex => ex.tags?.map(tag => tag.name).filter(Boolean) || []))), [initialRepairs]);
...
      currentFiltered = currentFiltered.filter(example => example.category?.name === selectedCategory);
    }

    if (selectedSymptoms.size > 0) {
      currentFiltered = currentFiltered.filter(example =>
        Array.from(selectedSymptoms).every(symptom => example.tags?.some(tag => tag.name === symptom))
      );
    }
...
          <div className="flex flex-wrap gap-3 justify-center sm:justify-start">
            <button
                ...
                className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 ease-in-out ${selectedCategory === 'all' ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-200 text-gray-700 hover:bg-blue-100 hover:text-blue-700'}`}
            >
                全て
            </button>
            {allCategories.map(category => (
                <button
                    ...
                    className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-200 ease-in-out ${selectedCategory === category ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-200 text-gray-700 hover:bg-blue-100 hover:text-blue-700'}`}
                >
                    {category}
                </button>
            ))}
